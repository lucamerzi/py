...
for i in jours_ord:
    ...
    select A.Jour Jour,A.Conversion_page_ID Conversion_page_ID, Audience, count(*) cnt
    from
        (select
        ...
        *
        from
            (SELECT *, row_number() over (partition by B.weboid, A.Conversion_page_ID order by B.dDataSource desc) Rang
            FROM 
                  (Select *
                  from  (select weboid, Audience,dDataSource
                        from table_date_range([datamining-1184:AXA_ES.AEW3_ProjRemade8_],timestamp('%s'),timestamp('%s')) #APP Projection sur le mois precedent
                        where Audience <> 1
                        )B

                  inner join

                        (select USER_ID, date(dh_serv) AS Jour, Conversion_page_ID
                        FROM [datamining-1184:AXA_ES.wcm_conversion_%s] #WCM pour le jour courant
                        WHERE action_type='conversion' AND Conversion_page_ID IN ('24', '31')
                        group by user_id, Jour, Conversion_page_ID
                        )A on B.weboid=A.user_id
                 )
            )
        where rang = 1)
    group by Jour, conversion_page_id, audience
    order by Jour, conversion_page_id, audience

    ############################## END QUERY
    
    """ %(target_date_93_BQ, target_date_BQ, target_date_BQ)
    
    table_destination_name = 'traffic_composition'
    # print(query)
